<?php 
    ...
    $id = $_GET['id'];
    $data = select("SELECT * FROM inventory_item WHERE id_item = $id");
        ...
        <div class="col-md-6 mx-auto mt-3">
            ...
            <form action="../functions/editPost.php" method="post" enctype="multipart/form-data">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <input type="text" name="item_name" id="item_name" class="form-control" required value="<?= $data[0]['item_name'] ?>">
                </div>
                <div class="d-flex align-items-center mb-3">
                    <label for="item_type" class="form-label me-5 col-md-1">Type</label>
                    <input type="text" name="item_type" id="item_type" class="form-control" required value="<?= $data[0]['item_type'] ?>">
                </div>
                <div class="d-flex align-items-center mb-3">
                    <label for="item_qty" class="form-label me-5 col-md-1">Quantity</label>
                    <input type="number" name="item_qty" id="item_qty" class="form-control" required value="<?= $data[0]['item_qty'] ?>">
                    </div>
                <div class="d-flex align-items-center mb-3">
                    <label for="item_desc" class="form-label me-5 col-md-1">Description</label>
                    <input type="text" name="item_desc" id="item_desc" class="form-control" required value="<?= $data[0]['item_desc'] ?>">
                </div>
                <div class="mb-3">
                    <label for="item_condition" class="form-label">Condition</label>
                    <select id="item_condition" class="form-select" name="item_condition" value="<?= $data[0]['item_condition'] ?>">
                ...
                <input type="submit" class="btn mt-3 fw-bold" style="color: #cdfadb;Background-color: #43847b" value="Update">
            </form>
        </div>
    </div>
